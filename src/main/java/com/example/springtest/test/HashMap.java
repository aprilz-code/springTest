...
//
//    /**
//     * 最大容量，如果其中一个构造函数使用参数隐式指定了更高的值，
//     * 则使用该容量。必须是2的幂<= 1<<30。
//     */
//    static final int MAXIMUM_CAPACITY = 1 << 30;
//
//    /**
//     * 在构造函数中没有指定负载因子时使用的负载因子。 默认0.75 即 12的时候触发扩容，16*2=32
//     */
//    static final float DEFAULT_LOAD_FACTOR = 0.75f;
//
//    /**
//     * 容器计数阈值，用于对容器使用树而不是列表。在至少有这么多节点的容器中添加元素时，
//     * 容器将被转换为树。
...
//     * 应小于TREEIFY_THRESHOLD，并且在去除时最多6个网格，有收缩检测。
//     */
...
//
//    /**
//     * 基本哈希bin节点，用于大多数条目。(参见下面的TreeNode子类，以及LinkedHashMap的Entry子类。)
//     */
...
//
//    /**
//     * 结构修改是指改变HashMap中映射的数量或以其他方式修改其内部结构(例如，rehash)。
//     * 该字段用于使HashMap的collection -view上的迭代器快速失败。
...
//                    if ((e = p.next) == null) {
//                        //TODO important 节点大于8，切treeifyBin内，数组长度大于64即由链表转红黑树
...
//
//    /**
//     * 返回此映射中包含的映射的Set视图。集合是由映射支持的，
//     * 所以对映射的更改会反映在集合中，反之亦然。
//     * 如果在对集合进行迭代时修改了映射(除了通过迭代器自己的remove操作，
//     * 或者通过迭代器返回的映射项的setValue操作)，迭代的结果是未定义的。
//     * set支持元素移除，即通过Iterator从映射中移除对应的映射。
//     * 删除组。remove、removeAll、retainAll和clear操作。不支持add或addAll操作。
//     *
